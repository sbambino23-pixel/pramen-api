...
async function initDb(): Promise<void> {
  ...
  try {
    ...
    console.log("Database initialized (circles + users + analytics)");
  } catch (err) { console.error("Failed to init database:", err); } finally { client.release(); }
}

// ─── Circle Cache ────────────────────────────────────────────────────

const circles = new Map<string, StoredCircle>();
async function loadAllFromDb(): Promise<void> { try { const r = await pool.query("SELECT code, data FROM circles"); for (const row of r.rows) circles.set(row.code, row.data as StoredCircle); console.log(`Loaded ${circles.size} circles`); } catch (err) { console.error("Load circles failed:", err); } }
...
async function pullPlausibleMetrics(): Promise<void> {
  ...
  try {
    ...
    await pool.query(`INSERT INTO daily_web_metrics (date,visitors,pageviews,bounce_rate,visit_duration_avg,app_store_clicks,top_sources,updated_at) VALUES ($1,$2,$3,$4,$5,$6,$7,NOW()) ON CONFLICT (date) DO UPDATE SET visitors=$2,pageviews=$3,bounce_rate=$4,visit_duration_avg=$5,app_store_clicks=$6,top_sources=$7,updated_at=NOW()`,
      [today, m.visitors?.value||0, m.pageviews?.value||0, m.bounce_rate?.value||0, m.visit_duration?.value||0, clicks, JSON.stringify(src)]);
...
app.get("/", (c) => c.json({ status: "ok", service: "prAmen API", version: "2.2.0", storage: "postgres", circles: circles.size, analytics: POSTHOG_API_KEY ? "posthog" : "disabled", plausible: PLAUSIBLE_API_KEY ? "✓" : "✗", apple: ASC_KEY_ID ? "✓" : "✗", dashboard: "/dashboard?key=...", timestamp: new Date().toISOString() }));
...
app.put("/api/user/data", async (c) => {
  const ah = c.req.header("Authorization"); if (!ah?.startsWith("Bearer ")) return c.json({ error: "Unauthorized" }, 401);
  const u = await getUserByToken(ah.replace("Bearer ", "")); if (!u) return c.json({ error: "Unauthorized" }, 401);
...
app.delete("/api/circles/:code/members/:userId", async (c) => { const code = c.req.param("code").toUpperCase(); const uid = c.req.param("userId"); const ci = getCircle(code); if (!ci) return c.json({ error: "Not found" }, 404); ci.members = ci.members.filter(m => m.userId !== uid); trackEvent(uid, "circle_left", { circle_code: code, remaining_members: ci.members.length }); ci.members.length === 0 ? await deleteCircleFromDb(code) : await saveCircleToDb(ci); return c.json({ success: true }); });
...
app.get("/api/dashboard/events", async (c) => {
  ...
  try {
    ...
    const res = await fetch(`${POSTHOG_HOST}/api/projects/${POSTHOG_PROJECT_ID}/events/?limit=${limit}&after=${afterDate}&orderBy=-timestamp`, { headers: { Authorization: `Bearer ${POSTHOG_PERSONAL_KEY}` } });
    if (!res.ok) return c.json({ error: "PostHog " + res.status }, 500);
  ...
  } catch (e: any) { return c.json({ error: "PostHog failed", detail: e.message }, 500); }
});
...
app.get("/api/dashboard/appstore", async (c) => {
  ...
  try {
    ...
    if (!r.ok) return c.json({ connected: false, error: "Apple API " + r.status });
...
app.get("/api/dashboard", async (c) => {
  ...
  try {
    ...
    const pd = await pool.query(`SELECT * FROM daily_product_metrics WHERE date >= CURRENT_DATE - INTERVAL '7 days' ORDER BY date DESC`).catch(() => ({ rows: [] }));
...
async function start() {
  ...
  serve({ fetch: app.fetch, port: PORT }, (info) => {
    ...
    console.log(`   PostHog: ${POSTHOG_API_KEY ? "✓" : "✗"} | Plausible: ${PLAUSIBLE_API_KEY ? "✓" : "✗"} | Apple: ${ASC_KEY_ID ? "✓" : "✗"}`);
