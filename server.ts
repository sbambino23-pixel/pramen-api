...
async function initDb(): Promise<void> {
  ...
  try {
    ...
    console.log("DB initialized (v4.0.5 — secure referral tracking, tier cleanup)");
...
async function evaluateTierRewards(userId: string): Promise<void> {
  ...
  const tiers = [{ tier: 1, threshold: 1, type: "7_days_free", days: 7 }, { tier: 2, threshold: 5, type: "1_month_free", days: 30 }, { tier: 4, threshold: 20, type: "free_lifetime", days: 0 }];
  for (const t of tiers) {
    if (count >= t.threshold) {
      ...
      if (REVENUECAT_SECRET_KEY && (t.tier === 2 || t.tier === 4)) {
...
async function start() {
  ...
  serve({ fetch: app.fetch, port: PORT }, (info) => {
    console.log(`\n🙏 prAmen API v4.0.5 on port ${info.port}`);
