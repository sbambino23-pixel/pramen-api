...
app.get("/api/circles/:code", async (c) => { const ci = getCircle(c.req.param("code")); if (!ci) return c.json({ error: "Not found" }, 404); try { const memberIds = ci.members.map(m => m.userId).filter(Boolean); if (memberIds.length > 0) { const avatars = await pool.query("SELECT id, device_user_id, avatar_url, name FROM users WHERE id = ANY($1) OR device_user_id = ANY($1)", [memberIds]); const avatarMap: Record<string, { avatar_url: string | null; name: string }> = {}; for (const row of avatars.rows) { avatarMap[row.id] = { avatar_url: row.avatar_url, name: row.name }; if (row.device_user_id) avatarMap[row.device_user_id] = { avatar_url: row.avatar_url, name: row.name }; } const visibleMembers = ci.members.filter(m => m.visible !== false || m.userId === ci.creatorUserId); const enriched = { ...ci, members: visibleMembers.map(m => ({ ...m, avatarUrl: avatarMap[m.userId]?.avatar_url || m.avatarUrl || null, name: avatarMap[m.userId]?.name || m.name })) }; return c.json({ circle: enriched }); } } catch (err: any) { console.error("[Circle] Avatar enrich error:", err.message); } return c.json({ circle: ci }); });
...
app.get("/api/dashboard/events", async (c) => {
  ...
  try {
    ...
    const stepOrder = ["welcome","language","topics","reminders","circle_tutorial","first_prayer_completed","first_prayer_skipped","sign_in","community_circles_joined","community_circles_skipped","circle_created","invite_story","saved_story","invite_code","invite_later","invite_accepted","paywall","converted"];
