...
app.get("/api/dashboard/events", async (c) => {
  ...
  try {
    ...
    const stepOrder = ["welcome","language","topics","reminders","sign_in","community_circles_joined","community_circles_skipped","circle_created","first_prayer_completed","first_prayer_skipped","invite_story","saved_story","invite_code","invite_later","invite_accepted","paywall","converted"];
...
app.get("/api/dashboard/revenuecat", async (c) => {
  ...
  try {
    ...
    const netTrialMonthly = subscribers.filter((s: any) => s.subscriptions.some((sub: any) => sub.is_active && sub.period_type === "trial" && !sub.unsubscribe_detected_at && sub.product_id?.includes("monthly"))).length;
    const netTrialYearly = subscribers.filter((s: any) => s.subscriptions.some((sub: any) => sub.is_active && sub.period_type === "trial" && !sub.unsubscribe_detected_at && sub.product_id?.includes("yearly"))).length;
    const netSubMonthly = subscribers.filter((s: any) => s.subscriptions.some((sub: any) => sub.is_active && sub.period_type !== "trial" && !sub.unsubscribe_detected_at && sub.product_id?.includes("monthly"))).length;
    const netSubYearly = subscribers.filter((s: any) => s.subscriptions.some((sub: any) => sub.is_active && sub.period_type !== "trial" && !sub.unsubscribe_detected_at && sub.product_id?.includes("yearly"))).length;
    const netSubLifetime = subscribers.filter((s: any) => s.subscriptions.some((sub: any) => sub.is_active && sub.period_type !== "trial" && !sub.unsubscribe_detected_at && sub.product_id?.includes("lifetime"))).length;
